...
class ImageAnalysisTool:
    ...
    def create_config_section(self):
        ...
        self.prompt_text = tk.Text(prompt_frame, font=(self.custom_font, 9), wrap=tk.WORD)
        ...
        bottom_frame.grid(row=4, column=0, columnspan=2, sticky=(tk.W, tk.E), padx=8, pady=5)
        ttk.Label(bottom_frame, text="💡 配置将自动保存", style='Info.TLabel').pack(side=tk.RIGHT)
    ...
    def create_batch_mode_content(self):
        ...
        # 批量处理结果显示区域
        self.batch_result_text = scrolledtext.ScrolledText(info_frame, height=12, font=(self.custom_font, 9), wrap=tk.WORD)
    ...
    def update_model_names(self):
        ...
        try:
            # 检查必要的组件是否存在
            if not hasattr(self, 'doubao_check') or not hasattr(self, 'xai_check'):
    ...
    def update_model_colors(self):
        ...
        try:
            ...
            elif selected_model == "xai" and hasattr(self, 'xai_check'):
                print("设置XAI按钮为绿色")
                style.configure('XAI.TRadiobutton', foreground='green')
                self.xai_check.configure(style='XAI.TRadiobutton')
            elif selected_model == "gptsapi" and hasattr(self, 'gptsapi_check'):
            ...
            elif selected_model == "siliconflow" and hasattr(self, 'siliconflow_check'):
                print("设置硅基流动按钮为绿色")
    ...
    def call_api(self, image_path, prompt):
        ...
        if selected_model == "doubao":
            ...
            return self.call_xai_api(image_path, prompt)
        elif selected_model == "gptsapi":
            return self.call_gptsapi_api(image_path, prompt)
        elif selected_model == "siliconflow":
            return self.call_siliconflow_api(image_path, prompt)
    ...
    def call_xai_api(self, image_path, prompt):
        """调用XAI API"""
        try:
            # 读取图片文件并编码为base64
            with open(image_path, 'rb') as f:
    ...
    def call_gptsapi_api(self, image_path, prompt):
        """调用 GPTsAPI (Gemini) API"""
        try:
            with open(image_path, 'rb') as f:
                image_data = base64.b64encode(f.read()).decode('utf-8')
    ...
    def call_siliconflow_api(self, image_path, prompt):
        """调用硅基流动API"""
        try:
            # 读取图片文件并编码为base64
            with open(image_path, 'rb') as f:
            ...
            if response.status_code == 200:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    # 优先获取content字段，如果没有则获取reasoning_content
                    if "message" in choice:
                        ...
                        content = _filter_siliconflow_box(content)
                        print(f"API调用成功，返回内容长度: {len(content)} 字符")
                        return content
                    else:
                        return f"API响应格式异常: {result}"
