...
class ImageComparisonTool:
    ...
    def auto_save_config(self):
        """自动保存配置"""
        try:
            if hasattr(self, "api_key_entry") and getattr(self, "_last_selected_model_for_api", None):
    ...
    def create_single_mode_content(self):
        ...
        images_container.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        image_a_frame.rowconfigure(1, weight=1)
        ...
        self.image_a_preview.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 6))
        ...
        image_b_frame.rowconfigure(1, weight=1)
        ...
        self.image_b_preview = ttk.Label(image_b_frame, text="点击选择图片B",
                                        background='#f8f9fa', relief='ridge',
                                        anchor='center', padding=12)
        self.image_b_preview.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 6))
    ...
    def update_image_preview(self, image_path, preview_label):
        ...
        try:
            ...
            max_width = max(preview_label.winfo_width() - 24, 280)
            max_height = max(preview_label.winfo_height() - 24, 260)
            ...
            info_text = f"{file_name}\n({file_size} KB, {original_size[0]}x{original_size[1]})"
    ...
    def auto_detect_image_b(self, image_a_path):
        """自动检测并加载对应的图片B"""
        try:
            # 若用户已手动选择/拖入有效的图片 B，不再扫描文件夹，避免误报「未找到对应的B文件夹」
            existing_b = self.image_b_entry.get().strip()
            if existing_b and os.path.isfile(existing_b) and self.is_image_file(existing_b):
                return
    ...
    def call_api(self, image_a_path, image_b_path, prompt):
        ...
        if selected_model == "doubao":
            ...
            return self.call_autodl_api(image_a_path, image_b_path, prompt)
        elif selected_model == "aliyun":
    ...
    def call_autodl_api(self, image_a_path, image_b_path, prompt):
        """调用 AutoDL OpenAI 兼容 API（双图，单条 user 多模态）"""
        try:
            import base64
            with open(image_a_path, 'rb') as f:
                image_a_data = base64.b64encode(f.read()).decode('utf-8')
            ...
            # 双图 base64 体积大、推理慢，适当延长读超时（连接 30s，读取 300s）
            ...
            return (
                f"API调用超时: {str(e)}。"
                "双图模式上传数据量较大，若多次超时请检查网络、缩小图片或稍后重试。"
            )
        except Exception as e:
            return f"API调用异常: {str(e)}"
    ...
    def generate_comparison(self):
        ...
        if selected_model == "doubao" and not self.config.get("doubao_api_key"):
            ...
            self.progress_var.set("错误 - 请先配置 AutoDL API Key")
            return
